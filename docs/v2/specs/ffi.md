...
* Passing or returning structs by value larger than 16 bytes, or with
  floating-point or nested fields. Integer-class `@repr(C)` structs up to 16
  bytes (one or two registers, or by reference on Windows x64) do cross by
  value; see `docs/v2/specs/std-ffi.md`.
